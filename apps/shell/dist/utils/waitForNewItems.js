...
exports.waitForNewItems = waitForNewItems;
/**
 ...
 */
async function waitForNewItems(page, selector, timeout = 10000) {
    const previousCount = await page.$$eval(selector, els => els.length);
    return new Promise(async (resolve, reject) => {
        try {
            const newItems = await page.waitForFunction((selector, previousCount) => {
                const items = document.querySelectorAll(selector);
                return items.length > previousCount ? Array.from(items) : false;
            }, { timeout }, selector, previousCount);
